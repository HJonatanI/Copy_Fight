...
2. Completar los siguientes campos:
	* **Repository name**: nombre del repositorio, por ejemplo **Copy_Fight**.
...
3. **Clonar** el **repositorio**:
	...
	* Esto creará una **carpeta Copy_fight** con el **repositorio** descargado.
...
* **MySQL**:
	...
	* Si se está en **Windows** es necesario configurar el **PATH**.
	* Crear una **base de datos básica**.
		...
		* Crear la **Base de Datos**: Una vez dentro de **MySQL**, ejecutar el comando para crear una **base de datos**. En este caso, la **base de datos** se llamará **copy_fight**.
    		```
		...
		* Luego, otorgar **permisos completos** a este **usuario** en la **base de datos**:
    		```
            GRANT ALL PRIVILEGES ON copy_fight.* TO 'tu_usuario'@'localhost';
    		FLUSH PRIVILEGES;
    		```
		* Verificar la Creación de la **Base de Datos**: Ejecutar el siguiente comando para asegurar de que la **base de datos** fue creada correctamente:
            ...
            SHOW DATABASES;
            ```
		    Deberías verse **copy_fight** en la **lista** de **bases de datos**.
...
2. Abrir el **repositorio** en **Sublime Text**:
	* En la **terminal**:
        ```
        subl .
        ```
	* Alternativamente, abrir **Sublime** y usar **File > Open Folder** para seleccionar la carpeta del **repositorio**.

## Parte 2: Crear la estructura básica del proyecto en Django. ##

### Paso 1: Crear un proyecto de Django en el entorno local. ###

1. Asegúrase de que el **entorno virtual** esté activado:
	```
...
3. Después de ejecutar el comando, se generarán algunos archivos y carpetas en el proyecto:
	* **copy_fight/** (directorio del proyecto)
	* **__init__.py**: hace que **Django** trate este directorio como un **módulo** de **Python**.
...
1. Abrir **settings.py** en el directorio **copy_fight**.
...
2. Después de ejecutar el comando, se verá una nueva carpeta llamada **game** con los siguientes archivos y directorios:
	...
	* **urls.py**: este archivo no está creado por defecto, se añadira más adelante para definir **rutas** específicas de la **aplicación game**.
...
Definir las **tablas** principales para el juego usando **modelos** de **Django** y reflejar estos **modelos** en nuestra **base de datos MySQL**.

### Paso 1: Planificación de los modelos. ###

Para organizar la estructura de datos del juego, se identificarán los elementos. Antes de utilizar los **modelos** que se crean en esta parte, tendremos que eliminar de la **base de datos** ciertos datos de la **migración** anterior ya que si no se hace se generara un error en el **orden** de la **migración**.
```
mysql -u root -p
...
**Django** ya incluye un sistema de **autenticación de usuarios**, se puede aprovechar esto y extender el **modelo** de **usuario predeterminado** de **Django** para agregar **atributos** específicos. Se deajara todo preparado para dicha modificacion si es necesaria.
...
Cada **personaje** puede tener varios **objetos** en su **inventario**.
...
Para registrar las batallas entre **personajes**, se puede crear un **modelo** que guarde los resultados de cada enfrentamiento.
...
3. Luego, incluir estas **rutas** en la configuración principal de URLs del proyecto. Ir al archivo **urls.py** en la carpeta principal del proyecto y añade la **ruta** para **game**:
	```
	from django.contrib import admin
	...
	``` 
	Con esta configuración, nuestras **rutas** quedarán accesibles como **/game/perfil/**, **/game/inventario/**, y **/game/batalla/**.
...
Crear las **plantillas HTML** para visualizar el **perfil** del **personaje**, el **invbentario**, y el resultado de la **batalla**.
...
1. Iniciar sesión con el **superusuario** http://127.0.0.1:8000/admin.

2. Crear un **usuario** y dos **characters** para relacionar (Uno con el **superusuario** y otro con un **usuario** normal) en la **base de datos** de **Django**.
